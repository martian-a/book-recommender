...
public class ProfilePublicationYear extends Profile {

	public ProfilePublicationYear(GoodreadsFeedback currFeedback) {
		super(currFeedback);
	}

	@Override
	public boolean apply(GoodreadsBook currBook) throws ReportNotFoundException {
		return super.apply(currBook, currBook.getPublicationYear());
	}

	@Override
	public boolean apply(GoodreadsBook currBook, Measure currMeasure) throws ReportNotFoundException {
		return super.apply(currBook, currMeasure, currBook.getPublicationYear());
	}

	/**
	 * Evaluates the publication year of the current book to see if it matches
	 * the year sought.
	 * 
	 * @param currBook
	 ...
	 * the year sought
	 * 
	 * @return true if the book's year of publication matches the value sought;
	 * false if not.
	 */
	@Override
	protected boolean evaluateAttribute(GoodreadsBook currBook, int sought) {

		Integer currYear = currBook.getPublicationYear();
	...
	/**
	 * Evaluates the publication year of the current book to see if it falls
	 ...
	 * 
	 * @return true if the length of the book's publication year matches the
	 * year sought; false if not.
	 */
	@Override
	protected boolean evaluateAttribute(GoodreadsBook currBook, int[] sought) {

		Integer currLength = currBook.getPublicationYear();
	...
	protected void generate() {
		...
		this.reports.add(this.generate(this.reviewsAll, Scope.ALL, "Publication Year (Edition, All)"));
		this.reports.add(this.generate(this.reviewsPopular, Scope.POPULAR, "Publication Year (Edition, Popular)"));
		this.reports.add(this.generate(this.reviewsUnpopular, Scope.UNPOPULAR, "Publication Year (Edition, Unpopular)"));
	...
	private Report generate(HashSet<GoodreadsReview> currReviews, Scope currScope, String currReportTitle) {
		...
		for (GoodreadsReview currReview : currReviews) {
			...
			if (currBook == null) {
				...
				// Get the publication year of this copy of the book
				Integer currYear = currBook.getPublicationYear();
